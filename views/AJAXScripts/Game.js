$(document).ready(function () {
    ...
    if(e.which == 13 && commentText != "") {
		...
		$.get('/addComment', {text: commentText, username: username, commentOn: gameName}, function (result) {
...
//Game like function
$("#like_game_btn").on('click', function () {
	var username = $('#username').text();
	var isLiked = $('#isGameLiked').text();
	...
	var gameName = $('#gameName').text();
		...
		if(isLiked == "notLiked")
		{
			...
			$.get('/likeGame', {username: username, gameName: gameName, numLikes: numLikes}, function (result) {
			...
			numLikes++;
			$('#like_game').attr("src","/images/heart.png");
			$('#isGameLiked').text("Liked");
		...
		if(isLiked == "Liked")
		{
			...
			$.get('/dislikeGame', {username: username, gameName: gameName, numLikes: numLikes}, function (result) {
			
			
        	});
			
			numLikes--;
			$('#like_game').attr("src","/images/heartempty.png");
			$('#isGameLiked').text("notLiked");
			$('#numLikes').text(numLikes)
		}
...
$("#add2list_btn").on('click', function () {
	var username = $('#username').text();
	var gameName = $('#gameName').text();
	var listName = prompt("Please enter list name");
	var cover = $('#cover').text();
		
			$.get('/addtoList', {username: username, gameName: gameName, listName: listName, cover: cover}, function (result) {
				
				console.log(result);
				
				if(result == false)
				alert("List could not be found");
        	});
...
$(window).on('load', function () {
		...
		//Check whether current user has liked the game
		$.get('/checkifLikedGame', {username: username, gameName: gameName}, function (result) {
			if(result)
			{
				$('#like_game').attr("src","/images/heart.png");
				$('#isGameLiked').text("Liked");
